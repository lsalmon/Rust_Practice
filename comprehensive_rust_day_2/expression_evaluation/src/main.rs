...
fn eval(e: Expression) -> Res {
    match e {
        Expression::Op { op, left, right } => {
            let op_code = op;
            let lval = if let Ok(lval) = eval(*left) {
                lval
            } else {
                return Err(String::from("no left value"));
            };
            let rval = if let Ok(rval) = eval(*right) {
                rval
            } else {
                return Err(String::from("no right value"));
            };
            match op_code {
                Operation::Add =>{
                    Ok(lval + rval)
