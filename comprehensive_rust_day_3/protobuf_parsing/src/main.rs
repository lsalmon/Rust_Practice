...
fn parse_field(data: &[u8]) -> Result<(Field, &[u8]), Error> {
    ...
    let (fieldvalue, remainder) = match wire_type {
        ...
        WireType::Len => {
            ...
            (FieldValue::Len(&remainder[0..length as usize]), &remainder[length as usize..])
        },
        WireType::I32 => (FieldValue::I32(i32::from_le_bytes(remainder[0..4].try_into().unwrap())), &remainder[4..]),
...
impl<'a> ProtoMessage<'a> for Person<'a> {
    fn add_field(&mut self, field: Field<'a>) -> Result<(), Error> {
        ...
        } else if field.field_num == 2 {
            self.id = field.value.as_u64()?;
        } else if field.field_num == 3 {
            self.phone.push(parse_message(field.value.as_bytes().unwrap()).unwrap());
        } else {
            return Err(Error::UnexpectedWireType);
        }
        Ok(())
